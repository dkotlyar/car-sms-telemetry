...
uint8_t rec_buffer[SIM868_BUFFER_BANKS][SIM868_CHARBUFFER_LENGTH];
...
char fileReadBuffer[SIM868_CHARBUFFER_LENGTH] = {0};
...
#warning SIM868 USART bridge enabled
    ...
    if (rec_buffer_ignore_message) {
        ...
        if (data == '\n' || data == '\r') {
            ...
            if (rec_buffer_index < SIM868_CHARBUFFER_LENGTH) {
...
void sim868_handle_buffer(void) {
    ...
    cli();
    uint8_t bank[SIM868_CHARBUFFER_LENGTH] = {0};
...
void sim868_post_async(const char *data) {
    if (httpBuffer[0] == 0) {
        memset(httpBuffer, 0, SIM868_CHARBUFFER_LENGTH);
